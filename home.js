...
fetch(url, {
  ...
  .then(data => {
    ...
    function processData(data) {
      ...
      for (let i = 0; i < filteredXps.length; i++) {
        let amount = filteredXps[i].amount;
        totalAmount += amount;
      }
      ...
      if (totalAmount >= 1000000) {
        totalAmount = totalAmount / 1000000; // Convertir en mégaoctets (MB)
        // totalAmount = Math.round(totalAmount);
        totalAmount = totalAmount.toFixed(2); // Limiter à deux décimales
        totalAmount = totalAmount.replace('.', ','); // Remplacer le point décimal par une virgule
        document.getElementById('xp').textContent = totalAmount + ' MB';
      }
       else {
        totalAmount = totalAmount / 1000;
        totalAmount = Math.round(totalAmount);
        document.getElementById('xp').textContent = totalAmount + ' KB';
      }
